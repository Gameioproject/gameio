...
The hero now fills the opening viewport while leaving a 36 dp scaled peek of the first Explore row. Hero covers use a 240 dp target height, and Explore covers use 156 dp, with existing display scaling. Platform tabs scroll away with the hero; the main header remains fixed. Empty Recommended and Favorites sections are omitted from Explore after platform and availability filters, while the explicit Favorites feed retains its empty-state explanation. Section changes preserve controller focus by section key and game ID.

Scrolling uses a lightweight cover layout for unfocused games, keeping video animation and its extra layers on the focused tile. Lazy rows and sections declare reusable content types, and section filtering is memoized across focus changes. Entering a row restores its saved horizontal position without starting an animation; only focused selection changes scroll it. Visible controller targets use a bounded 200 ms scroll, with offscreen targets positioned directly.

Performance check on the Android emulator (same eight swipe gestures, warm covers, no build running): 90th-percentile frame time improved from 24 ms to 20 ms. Deadline misses were essentially unchanged (5.35% to 5.43%), so this is an improvement in rendering overhead rather than proof that all device-specific stutter is eliminated.
